...
//  ,paramsuperdeluxe:	[]
 ...
 var initialize = function(jQueryObj){
  // image preload
  image_load(settings.imageBtnPrev,  function(){settings.imageBtnPrevWidth  = (this.width > 0 ? this.width : 63);});
  image_load(settings.imageBtnNext,  function(){settings.imageBtnNextWidth  = (this.width > 0 ? this.width : 63);});
  image_load(settings.imageBtnClose, function(){settings.imageBtnCloseWidth = (this.width > 0 ? this.width : 66);});
  if (!frameBorder) {
 ...
 var set_video_info = function(contentNo) {
  ...
  jQuery.each(contentInfo.href.replace(/^.*\?/,'').split("&"), function(){
 ...
 var image_load = function(src, callback){
  ...
  if (typeof callback == 'function') objImageLoader.onload = callback;
