...
//	$cnt = count($comments);
	$cnt = get_comments_number() - trackpings('count');
...
function listtrackpings($type = 'both', $template='<li id="trackback-%id"><a href="%url">%origin</a><br />%content</li>', $dateformat='') {
	...
	if ($trackbacks = trackpings("", $type)) {
		foreach ($trackbacks as $trackback) {
			...
			$vars = array(
				$trackback->comment_ID,
				$trackback->comment_author_url,
				$trackback->comment_author,
				$trackback->comment_content,
				mysql2date($dateformat, $trackback->comment_date),
				);
