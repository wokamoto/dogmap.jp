...
if (ks_is_comments_list()) : 
	...
	if (isset($_GET['co_order']) && $_GET['co_order'] == 'asc') {
		$co_order = 'asc';
		$desc_format = '<a href="' . attribute_escape(add_query_arg('co_order', 'desc')) . '">' . __('From Newer', 'redportal') . '</a>';
		...
		$asc_format  = '<a href="' . attribute_escape(add_query_arg('co_order', 'asc')) . '">' .__('From Older', 'redportal') . '</a>';
	...
	foreach ($sep_comments as $type => $_comments) :
		...
		if (count($_comments)) : ?>
			<ol>
			<?php if ($walker) :
				wp_list_comments(array('walker' => $walker, 'callback' => 'ks_list_comments', 'avatar_size' => 0), $_comments);
				...
				foreach ($_comments as $comment) : ?>
					<li><a name="comment-<?php comment_ID(); ?>"></a> <?php ks_comment_content(); ?></li>
				<?php endforeach;
			endif; // $walker ?>
			</ol>
...
elseif (ks_is_comment_post()) : ?>
	...
	<?php } elseif (get_option('comment_registration') && ! is_user_logged_in()) {
		if (ks_admin_url(KTAI_NOT_ECHO)) { ?>
			<p><?php printf(__('You must be <a href="%s">logged in</a> to post a comment.'), ks_siteurl(KTAI_NOT_ECHO) . 'wp-login.php?redirect_to=' . urlencode(ks_comments_post_url()));?></p>
		<?php } else { ?>
		...
		if (isset($ks_commentdata['message']) && $ks_commentdata['message']) {
			...
			<?php echo implode('</font></li><li><font color="blue">', array_map('wp_specialchars', explode("\n", $ks_commentdata['message']))); ?>
			</font></li></ul>
		<?php }
		...
		if ( is_user_logged_in() ) {
			...
			if (ks_admin_url(KTAI_NOT_ECHO)) {
				?><div align="right"><img localsrc="120" alt="" /><?php printf(__('<a href="%s">Log in</a> and post a comment.', 'ktai_style'), ks_siteurl(KTAI_NOT_ECHO) . 'wp-login.php?redirect_to=' . urlencode(ks_comments_post_url()));?></div><?php 
			} 
